...
////////////////////////
// jQuery biButtonGroup plugin

(function ($) {

    var name = 'biButtonGroup', // Plugin name

	settings = {
	    data: [], // [{ label: '', value: '', checked: false }, ...]

	    rootCss: 'bi-form-button-group',
	...
	prototype = {
	    ...
	    init: function () {
	        ...
	        this.handler = $.biForm.input.handleEvents(this.$element, this.getCss('active'), this.options.uncheckRadio);
		...
		initMarkup: function () {
		    $.biForm.removeTextNodes(this.$element.get(0));
		    this.handler();
		}
	...
	methods = {
	    ...
	    build: function (data, adapter) {
	        ...
	        var nameAttr = this.getCss($.biForm.uniqueId()), nameIndex = 1;
	        (this.options.data = data).forEach(function (item, index) {

	            this.$element.append($.biForm.input.build({
	    ...
	    getSelection: function (part, adapter) { // part = 'label', 'value', or undefined
	        return $.biForm.input.getSelection(this.$element, part, adapter);
	    },

	    setSelection: function (items, matchLabel) {
	        return $.biForm.input.setSelection(this.$element, items, matchLabel, this.options.uncheckRadio);
	    }
