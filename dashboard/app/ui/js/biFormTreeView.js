...
/////////////////////
// jQuery biTreeView plugin

(function ($) {

    var name = 'biTreeView', // Plugin name
	...
	settings = {
	    ...
	    rootCss: 'bi-form-tree-view',
	    parentTag: 'ul',
	    childTag: 'li',

	    animDuration: 200 // should be same as the duration of .bi-form-tree-view-toggle transition (defined in css)
	},
	...
	prototype = {
	    ...
	    init: function () {
	        ...
	        this.$root.addClass(this.options.rootCss).on($.biPlugin.events('mousedown'), this.handleToggle.bind(this));
	        this.handler = $.biForm.input.handleEvents(this.$root, this.getCss('active'), this.options.uncheckRadio);
	    ...
	    buildDropDown: function () {
	        ...
	        this.$trigger.on($.biPlugin.events('mousedown'), function () {
	    ...
	    initMarkup: function () {
	        ...
	        var oneLevel = !this.$root.children(this.options.childTag).children(this.options.parentTag).size();
	    ...
	    traverse: function ($parent) {
	        ($parent || this.$root).children(this.options.childTag).each(function (index, child) {
	            $.biForm.removeTextNodes(child);
	            var $child = $(child), $checked = $child.children('label').children('input:checked');
	            if ($checked.size()) this.callMethod('expand', $checked);
	    ...
	    addThreeStates: function ($parentTag) {
	        ...
	        $3states.on($.biPlugin.events('mousedown'), function (e) {
	    ...
	    setData: function (data, adapter) {
	        ...
	        data.forEach(function (_item) {
	            ...
	            item.value = $.biForm.input.value2String(_item[adapter.value]);
	            if (!!~$.inArray(_item[adapter.parentValue], [undefined, false, null])) {
	                item.parentValue = null; // root item
	            } else {
	                item.parentValue = $.biForm.input.value2String(_item[adapter.parentValue]); // child item
	    ...
	    get$leave: function (item) {
	        ...
	        if (item.checkable) {
	            $leave.append($.biForm.input.build({
	                label: item.label,
	                value: item.value,
	    ...
	    toggleChilds: function ($childTags, toOpen) {
	        $childTags.each(function (index, childTag) {
	            ...
	            if (!$toggle.size()) return;
	            if (undefined === toOpen || !toOpen && isOpen || toOpen && !isOpen) {
	                $toggle.trigger($.biPlugin.events('mousedown'));
	            }
	        }.bind(this));
	    },
	    ...
	    addTool: function (action) {
	        ...
	        switch (action) {
	            case 'close':
	                $action.addClass(cssReverse).on($.biPlugin.events('mousedown'), function () {
	                ...
	                break;
	            case 'uncheck':
	                $action.on($.biPlugin.events('mousedown'), function () {
						var items = null; // Uncheck all items
						if ('checkbox' === this.options.type && !this.$root.find('input:checked').size()) {
	...
	methods = {
		...
		build: function (data, adapter) {
			...
			this.nameAttr = this.nameAttr || this.getCss($.biForm.uniqueId()); // unique for the instance
		...
		getSelection: function (part, adapter) { // part = 'label', 'value', or undefined
		    return $.biForm.input.getSelection(this.$root, part, adapter);
		},
		...
		setSelection: function (items, matchLabel, adaptView) {
		    ...
		    var $checked = $.biForm.input.setSelection(this.$root, items, matchLabel, this.options.uncheckRadio, clickHandler);
